...
data_Subset = column_to_rownames(data_Subset, var = "Sample")
...
Means <- data_Numeric %>% group_by(Factor) %>% summarise_each(funs(mean))
...
Means_T = rownames_to_column(Means_T, "Metabolite")

#Compute standard error
SE <- data_Log %>% group_by(Factor) %>% summarise_each(funs(std.error))
SE <- column_to_rownames(SE, "Factor")
SE_t = as.data.frame(t(SE))
SE_t = SE_t[-1,]
SE_t = rownames_to_column(SE_t, response_variable)
