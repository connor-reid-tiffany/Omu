...
```
DF_anova <- omu_anova(data = c57_nos2KO_mouse_countDF, colData = c57_nos2KO_mouse_metadata, response_variable = "Metabolite", var1 = "Background", var2 = "Treatment", interaction = TRUE, log_transform = TRUE, p_adjust = "BH")
```
...
```{r echo=FALSE,message=FALSE, warning=FALSE}
DF_anova <- omu_anova(data = data, colData = c57_nos2KO_mouse_metadata, response_variable = "Metabolite", var1 = "Background", var2 = "Treatment", interaction = TRUE, log_transform = TRUE, p_adjust = "BH")
DF_anova_trunc = DF_anova[,c(1:7)]
kable(DF_anova_trunc[1:3,])
```

The output givescolumns with adjusted p values for ```var1```, ```var2```, and your interaction term.
...
```
DF_stats_grouped <- omu_summary(data = c57_nos2KO_mouse_countDF, colData = c57_nos2KO_mouse_metadata, numerator = "WTStrep", denominator = "WTMock", response_variable = "Metabolite", Factor = "Grouped", log_transform = TRUE, p_adjust = "BH")
```

Producing this data frame:

```{r echo=FALSE,message=FALSE, warning=FALSE}
DF_stats_grouped <- omu_summary(data = data, colData = c57_nos2KO_mouse_metadata, numerator = "WTStrep", denominator = "WTMock", response_variable = "Metabolite", Factor = "Grouped", log_transform = TRUE, p_adjust = "BH")
...
```{r echo=FALSE,message=FALSE, warning=FALSE}
DF_s_trunc_g_a <- assign_hierarchy(data = DF_s_trunc_g, keep_unknowns = TRUE, identifier = "KO_Number")
...
```{r echo=FALSE,message=FALSE, warning=FALSE}
DF_s_trunc_g_a_sub_genes_prok <- assign_hierarchy(data = DF_s_trunc_g_a_sub_genes, keep_unknowns = FALSE, identifier= "Prokaryote")
...
```
DF_stats_counts <- count_fold_changes(data = DF_stats, "Class", column = "Class", sig_threshold = 0.05)
```
...
```{r echo=FALSE}
DF_stats_counts <- count_fold_changes(data = DF_stats, "Class", column = "Class", sig_threshold = 0.05, keep_unknowns = FALSE)
...
```
Volcano_Plot <- plot_volcano(data = DF_stats, size = 2, column = "Class", strpattern = c("Organic acids, Carbohydrates"), fill = c("firebrick2","white","dodgerblue2"), color = c("black", "black", "black"), alpha = c(1,1,1), shape = c(21,21,21)) + theme_bw() + theme(panel.grid = element_blank())
```
...
```
PCA <- PCA_plot(data = c57_nos2KO_mouse_countDF, colData = c57_nos2KO_mouse_metadata, variable = "Treatment", color = "Treatment", response_variable = "Metabolite")+ theme_bw() + theme(panel.grid = element_blank())
```
